(function () {
  ...
  const tick = async () => {
    ...
    // すべての.right-controls-buttonsコンテナにIMMERSIONボタンを追加
    const allRightControls = document.querySelectorAll('.right-controls-buttons');
    allRightControls.forEach((rc, index) => {
      // このコンテナにボタンがあるか確認
      let toggleBtn = rc.querySelector('.my-mode-toggle-btn');
      if (!toggleBtn) {
        toggleBtn = createEl('button', '', 'my-mode-toggle-btn', 'IMMERSION');
        // 最初のボタンにはIDも設定（互換性のため）
        if (index === 0) {
          toggleBtn.id = 'my-mode-toggle';
        }
        ...
        toggleBtn.onclick = () => {
          ...
          // すべてのボタンの状態を同期
          document.querySelectorAll('.my-mode-toggle-btn').forEach(btn => {
    ...
    // 後方互換性のため、古いIDでのボタン参照も維持
    let toggleBtn = document.getElementById('my-mode-toggle') || document.querySelector('.my-mode-toggle-btn');
